...
namespace Enemy2 
{
	Object::Object(CManager* pManager)
	{
		...
		manager = pManager;
		//画像
		LoadDivGraph("./data/image/slime.png", 4, 4, 1, 300, 300, graphic);
		//ポジション
	...
	void Object::Render()
	{
		DrawExtendGraph(this->pos.x, this->pos.y, this->pos.x + 150, this->pos.y + 150, graphic[this->Anim()], TRUE);
	...
	void Object::Think()
	{
		...
		case Motion::BeforeAttack:
			...
			break;
			case 1:
				AttackLoad("./data/attack/stage1/Stamp.txt");
				if (this->moveCnt / 60 > 1.2f) { nm = Motion::Attack2; this->moveCnt = 0; }

				break;
			case 2:
				AttackLoad("./data/attack/stage1/LeftAttack.txt");
				if (this->moveCnt / 60 > 1.2f) { nm = Motion::Attack3; this->moveCnt = 0; }

				break;
			case 3:
				AttackLoad("./data/attack/stage1/ReverseCross.txt");
		...
		case Motion::Attack:
			...
			if (this->moveCnt / 60 > 0.5f) { nm = Motion::Stand; AttackKey = 1; this->moveCnt = 0;  }
			if (this->Hp <= 0) { nm = Motion::Dead; this->moveCnt = 0; }
			break;
		case Motion::Attack2:
			preattackkey = AttackKey;
			if (this->moveCnt / 60 > 0.5f) { nm = Motion::Stand; AttackKey = 2; this->moveCnt = 0;}
			if (this->Hp <= 0) { nm = Motion::Dead; this->moveCnt = 0; }
			break;
		case Motion::Attack3:
			preattackkey = AttackKey;
			if (this->moveCnt / 60 > 0.7f) { nm = Motion::Stand; AttackKey = 3; this->moveCnt = 0;  }
			if (this->Hp <= 0) { nm = Motion::Dead; this->moveCnt = 0; }
			break;
		case Motion::Attack4:
			preattackkey = AttackKey;
